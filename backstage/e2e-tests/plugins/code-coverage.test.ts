...
test('Code Coverage plugin card on catalog entity', async ({ backstage }) => {
  ...
  await backstage.content
    .getByRole('link', { name: 'code-coverage-example' })
    .click();

  await expect(
    backstage.header.getByText('code-coverage-example'),
  ).toBeVisible();

  await expect(backstage.content.getByText('Code Coverage')).toBeVisible();
});
